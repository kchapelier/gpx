...
var GpxFile = (function() {
    ...
    GpxFile.prototype.contents = null;

    GpxFile.prototype.add = function(content) {
    ...
    GpxFile.prototype.setTime = function(time) {
        if(typeof time.toISOString !== 'function') {
            throw new Error('arguments must be an object with a toISOString method');
        }
    ...
    GpxFile.prototype.getXml = function() {
        ...
        if(this.time) {
            xml.push('    <time>' + enc(this.time.toISOString()) + '</time>');
        }
